...
for (var i = 0; i < informationTitleArray.length; i++){
    ...
    for(var j = 0; j < childNodeArray.length; j++){
        if ( childNodeArray[j].nodeName == "TD" ){
            if(childNodeArray[j].childNodes.length > 0 && childNodeArray[j].childNodes[0].nodeName == "IMG"){
                if (j+2 < childNodeArray.length){
                    informationTitleDataArray.push(childNodeArray[j+2].childNodes[0].innerText)
...
for (var i = 0; i < informationDetailsArray.length; i++){
    var detailesChildNodeArray = Array.from(informationDetailsArray[i].childNodes)
    for(var j = 3; j < detailesChildNodeArray.length; j+=4){
        ...
        else if(informationDetailsArray[i].className == "smallerFont companyCriticism"){
